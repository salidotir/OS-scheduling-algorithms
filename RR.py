...
class RR:
    ...
    def run(self):
        ...
        while len(self.tmp_queue) != 0:
            ...
            if int(value[2]) != 0:
                if value[2] <= self.time_quantum:
                    self.gantt.append(
                        (
                            value[1], self.current_time,
                            self.current_time + int(value[2])))  # (p_id, start_time, end_time)
                    ...
                    self.gantt.append(
                        (value[1], self.current_time,
                         self.current_time + self.time_quantum))  # (p_id, start_time, end_time)
                    self.current_time += self.time_quantum
                    self.tmp_queue.append((self.current_time, value[1], value[2] - self.time_quantum, value[3]))
                    time.sleep(self.time_quantum / 1000)  # time to run process
